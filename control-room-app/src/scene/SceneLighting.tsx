...
/**
 * Museum-grade cinematic lighting with HDR environment,
 * contact shadows, volumetric beam cones, and physically
 * motivated light placement.
 */
...
export function SceneLighting() {
  ...
  return (
    <>
      {/* ── HDR Environment (custom HDRI from Poly Haven, CC0) ── */}
      <Environment
        files={HDRI_PATH}
        background={false}
        environmentIntensity={isNight ? 0.15 : 0.35}
      />
      ...
      {/* ── Rim / back light — warm cinematic edge ── */}
      <spotLight
        ref={rimRef}
        position={[0, 6, -8]}
        angle={0.5}
      ...
      {!performanceMode && (
        <>
          <VolumetricBeam position={[-3, 4.5, -1]} color="#f0e8d0" intensity={0.15} />
          <VolumetricBeam position={[0, 4.5, 0]} color="#f0e8d0" intensity={0.12} />
          <VolumetricBeam position={[3, 4.5, 1]} color="#f0e8d0" intensity={0.1} />
        </>
      )}
      ...
      <pointLight
        position={[0, 1.2, -0.2]}
        intensity={emergency ? 0.8 : 0.5}
        color={emergency ? '#ff2020' : '#70c0a0'}
        distance={5}
        decay={2}
      />

      {/* ── AutoStore area accent ── */}
      <pointLight position={[3.2, 2.5, -1.5]} intensity={0.4} color="#7090b0" distance={7} decay={2} />

      {/* ── Conveyor area accent ── */}
      ...
      <RimStrip position={[-8, 0.05, 0]} rotation={[0, 0, 0]} width={0.05} height={14} color="#c8a868" intensity={0.12} />
      <RimStrip position={[8, 0.05, 0]} rotation={[0, 0, 0]} width={0.05} height={14} color="#c8a868" intensity={0.12} />
      <RimStrip position={[0, 0.05, -7]} rotation={[0, 0, 0]} width={16} height={0.05} color="#8090b0" intensity={0.08} />
      <RimStrip position={[0, 0.05, 7]} rotation={[0, 0, 0]} width={16} height={0.05} color="#8090b0" intensity={0.08} />
    </>
  );
}

/** Fake volumetric beam using a semi-transparent cone */
...
/** Rim glow strip for pedestal edges */
function RimStrip({ position, rotation, width, height, color, intensity }: {
  position: [number, number, number];
