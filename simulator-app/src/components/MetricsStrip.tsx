  ...
  return (
    <section className="subpanel">
      <div className="mb-3 flex items-baseline justify-between">
        <span className="text-[0.7rem] uppercase tracking-[0.16em] text-[var(--as-text-sub)]">Efficiency Index</span>
        <strong className="text-2xl font-semibold text-[var(--as-accent-strong)] tabular-nums">
      ...
      <ul className="grid gap-1.5">
        {items.map((item) => (
          <li key={item.label} className="metric-row">
            ...
            <span className="tabular-nums text-[var(--as-text-main)]"><AnimatedValue value={item.value} digits={item.digits ?? 1} suffix={item.suffix ?? ''} /></span>
          </li>
        ))}
      </ul>
    </section>
